...
export class NoOpAuditLogger implements IAuditLogger {
  private readonly emptyResult: IAuditQueryResult = {
    entries: [],
    total: 0,
    offset: 0,
    hasMore: false,
  ...
  async query(): Promise<IAuditQueryResult> {
    return this.emptyResult;
  }

  async getByUser(): Promise<IAuditQueryResult> {
    return this.emptyResult;
  }

  async getByTarget(): Promise<IAuditQueryResult> {
    return this.emptyResult;
  }

  async getSummary(): Promise<IAuditSummary> {
    return {
      totalEntries: 0,
      byAction: {},
      bySeverity: {
  ...
  private createEmptyEntry(options: ICreateAuditEntryOptions): IAuditEntry {
    return {
      id: '',
