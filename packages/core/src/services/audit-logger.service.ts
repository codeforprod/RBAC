...
import { AuditOptions, DEFAULT_AUDIT_OPTIONS } from '../types/options.types';

/**
 * In-memory storage for audit entries (for development/testing).
 */
interface InMemoryAuditStorage {
  entries: IAuditEntry[];
  maxEntries: number;
}
...
export class InMemoryAuditLogger implements IAuditLogger {
  private readonly storage: InMemoryAuditStorage;
  ...
  constructor(
    ...
    this.options = { ...DEFAULT_AUDIT_OPTIONS, ...config.auditOptions };
  }
  ...
  async logPermissionCheck(
    ...
    return this.log({
      ...
      severity: granted ? AuditSeverity.INFO : AuditSeverity.ERROR,
      actorId: userId,
      permission,
   ...
   */
  private createEntry(options: ICreateAuditEntryOptions): IAuditEntry {
    this.idCounter++;
    return {
   ...
   */
  private addEntry(entry: IAuditEntry): void {
